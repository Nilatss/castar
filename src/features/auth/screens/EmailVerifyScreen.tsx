/**
 * Castar — Email Verify Screen
 *
 * Shown after EmailAuth. User enters the 4-digit verification code
 * sent to their email, then proceeds to SetName -> SetPin.
 *
 * Layout matches SetPinScreen: custom keypad, PIN-style cells (42x51, dot indicator).
...
import { verifyEmailCode } from '../services/emailAuth';
...
// Letter/envelope icon — 24x24
const letterIconSvg = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const glowSvg = `<svg width="${GLOW_RENDER_SIZE}" height="${GLOW_RENDER_SIZE}" viewBox="0 0 ${GLOW_RENDER_SIZE} ${GLOW_RENDER_SIZE}" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <radialGradient id="ev1" cx="0.5" cy="0.5" r="0.5">
  ...
  <ellipse cx="${GLOW_RENDER_SIZE / 2}" cy="${GLOW_RENDER_SIZE / 2}" rx="${GLOW_RENDER_SIZE / 2}" ry="${GLOW_RENDER_SIZE / 2}" fill="url(#ev1)"/>
</svg>`;
...
const glow2Svg = `<svg width="${GLOW2_RENDER_SIZE}" height="${GLOW2_RENDER_SIZE}" viewBox="0 0 ${GLOW2_RENDER_SIZE} ${GLOW2_RENDER_SIZE}" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <radialGradient id="ev2" cx="0.5" cy="0.5" r="0.5">
      <stop offset="0" stop-color="#FFFFFF" stop-opacity="0.16"/>
      <stop offset="0.05" stop-color="#FFFFFF" stop-opacity="0.15"/>
      <stop offset="0.12" stop-color="#FFFFFF" stop-opacity="0.13"/>
      <stop offset="0.20" stop-color="#FFFFFF" stop-opacity="0.11"/>
  ...
  <ellipse cx="${GLOW2_RENDER_SIZE / 2}" cy="${GLOW2_RENDER_SIZE / 2}" rx="${GLOW2_RENDER_SIZE / 2}" ry="${GLOW2_RENDER_SIZE / 2}" fill="url(#ev2)"/>
</svg>`;
...
const GLOW1_GREEN = buildVividGlowSvg(GLOW_RENDER_SIZE, 'evg1g', SUCCESS_GREEN);
const GLOW1_RED = buildVividGlowSvg(GLOW_RENDER_SIZE, 'evg1r', ERROR_RED);
const GLOW2_GREEN = buildVividGlowSvg(GLOW2_RENDER_SIZE, 'evg2g', SUCCESS_GREEN);
const GLOW2_RED = buildVividGlowSvg(GLOW2_RENDER_SIZE, 'evg2r', ERROR_RED);
...
export const EmailVerifyScreen = () => {
  ...
  const route = useRoute<RouteProp<AuthStackParamList, 'EmailVerify'>>();
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      // Fade in text for within-screen phase transition
      Animated.parallel([
  ...
  // Refs for stable callbacks — declared before handleVerify so it can use isSavingRef
  ...
  // Uses refs for isSaving to avoid stale closure — handleVerify is captured by
  // handleDigitPress via setTimeout, so it must stay stable across re-renders.
  const handleVerify = useCallback(async (enteredCode: string) => {
    ...
    try {
      ...
      // IMPORTANT: Show green success animation FIRST, then persist auth.
      // loginWithEmail() may set isOnboarded=true for returning users, which
      // instantly swaps RootNavigator to PinLock/Main and destroys this screen.
      // We must let the user see the success feedback before that happens.
      ...
      showSuccessState(() => {
        Animated.timing(screenContentOpacity, {
          ...
          if (token) {
            await useAuthStore.getState().loginWithEmail(token, email);
          }
          // If loginWithEmail set isOnboarded (returning user), RootNavigator
          // will switch to PinLock automatically. Otherwise navigate to SetName.
          const { isOnboarded } = useAuthStore.getState();
          if (!isOnboarded) {
            navigation.navigate('SetName', { from: 'email' });
          }
        });
      });
    } catch {
  ...
  }, [email, navigation, screenContentOpacity, showSuccessState, showErrorState]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.screenContent, { opacity: screenContentOpacity }]}>
        {/* Upper section (header + PIN cells + resend) */}
        <View style={styles.inner}>
          {/* Header block: icon + title + subtitle with email highlighted */}
          <Animated.View style={[styles.headerBlock, { opacity: textOpacity }]}>
            <SvgXml xml={letterIconSvg} width={24} height={24} />
